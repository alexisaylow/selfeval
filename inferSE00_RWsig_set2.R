...
# inferSE00_RWsig_set2 is for doing the original RW model for eLife set complete with wexp on the cluster.
# but with sigmoid response function
# ON THE CEDRIC VM, SETWD TO setwd("/media/sf_CHERhomeYdrive/R/R4hal") BEFORE TESTING THIS!!
#
# Alexis (An Yee) Low & Michael Moutoussis, 2020
...
if (whoami == 'kikidi'){ 
  ...
  outDir <- '/media/sf_CHERhomeYdrive/R/R4hal/currentWork/socioRWsig_set2/' };
if (whoami == 'LinuxMM'){ 
  codeDir <- '~/Dropbox/FIL_aux/R_scripts/';
  outDir <- baseDir };
if (whoami == 'MMhal9000'){
  codeDir <- '~/R/R4hal/scripts/';
  outDir <- '~/R/R4hal/currentWork/socioRWsig_set2/' };
...
npar = length(parhd)
try({load(paste(baseDir,"loadfornlm_RW_sigmoid_set2.RData",sep='')); #change this depending on which set
  bestsofar <- importCSV(paste(baseDir,"ml0res1cnnp.csv",sep=''))})  
#this contains bestsofar9, tryPmatrix, datArW61, and priors
try({load(paste(baseDir,"/currentWork/loadfornlm_RW_sigmoid_set2.RData",sep='')); 
  bestsofar <- importCSV(paste(baseDir,"currentWork/ml0res1cnnp.csv",sep=''))}) # for kikidi / hal ?
