...
try(remove(whoami,codeDir,baseDir,seDirs,bestsofar9)) # conservative clearup
...
SLPsocio4 <- function(parMat,
  ...
  for (ptN in 1:Nptot) {
    ...
    accP <- (parMat[ptN,'a0min']+parMat[ptN,'a0max'])/2/n
...
tr2natLP4 <- function(trp,check=1){  # from transformed, i.e. -inf to inf, to native space
  ...
  p[,5] <- 0.5*(1 + tanh(trp[,5])) 
  n <- 2
  p[,2] <- 0.5*n*(tanh(trp[,2])+1) # Or 1+0.5*(n0-1)*(tanh(trp[,2])+1)
...
nat2trLP4 <- function(p,check=1){  # From native to transformed.
  ...
  #       atanh(2*a0min/a0max-1), atanh(2*a0max/n0-1), ln(Tpred), Bpred, atanh(2decayCoeffGroups-1),
  # atanh(2decayCoeffSelf-1), ln(weightSelf), ln(sensi), ln(sesh))
  ...
  trp[,2] <- atanh(2*p[,2]/n   -1); # OR, if > 1, atanh(2*(p[,2]-1)/(n0-1)-1)
...
msLP4tr <- function(trParM, datAr, Pri=NA, check=0){
  ...
  if (length(Pri)>1){  # legit prior must have 20 elements
    for (ptN in 1:dim(trParM)[1]) {
      #            'a0min', 'a0max', 'Tpred', 'Bpred ', 'decayCoeffGroups', 'decayCoeffSelf', 'weightSelf', 'sensi', 'sesh'
      #             gamma    gamma    gamma    norm      beta                  beta         gamma      gamma     gamma
      # First the non-gamma-prior param:
      mSLPrior <- 
        -dbetaMS(parM[ptN,5], Pri[1,5],Pri[2,5],log=TRUE) #remove this line if 10p
...
try({load(paste(baseDir,"loadfornlm9.RData",sep=''));
  bestsofar9 <- importCSV(paste(baseDir,"soc04fit9par01to61_1.csv",sep=''))}) #this contains bestsofar9, tryPmatrix, datArW61, and priors
try({load(paste(baseDir,"/currentWork/loadfornlm9.RData",sep='')); 
  bestsofar9 <- importCSV(paste(baseDir,"/currentWork/soc04fit9par01to61_1.csv",sep=''))}) # for kikidi / hal ?
...
beliefModelFit <- function(pts,Par0=NULL,nlmprintlev=0) {
  ...
  dimnames(ml1res)[[2]] <- c('a0min', 'a0max', 'Tpred', 'Bpred', 'decayCoeffGroups', 'decayCoeffSelf', 'weightSelf', 'sensi', 'sesh', 'predSLnP', 'SESLnP','SEcor','predProb','BIC');
  ...
  for (ptN in pts){ #1:dim(datArW61)[3] ){
    ...
    ml1res[ptN,13]   <- exp(ml1res[ptN,10]/dim(datArW61[,,ptN])[1])  #cross-entropy 
