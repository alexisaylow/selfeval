...
# inferSE00_set1 is for doing the original RW model for eLife set complete with wexp on the cluster.
...
if (runOnCluster == 0){
   ...
   gridAtt = round((1:10)*12.34);   # selection of e.g. 10 gird attempts, max is 1:128 below .... 
} else {  # if run on cluster
  gridAtt = 1:128;
}

# Next, determine wich pt(s) to run:
if (Debug & !(runOnCluster)){ participant = 3; gridAtt = c()};    #A: changing this when testing - remember to change back             
# If running serially:
if (runOnCluster == 0){
...
if (whoami == 'kikidi'){ 
  ...
  outDir <- '/media/sf_CHERhomeYdrive/R/R4hal/currentWork/socio00cnnp/' };
if (whoami == 'LinuxMM'){ 
  codeDir <- '~/Dropbox/FIL_aux/R_scripts/';
  outDir <- baseDir };
if (whoami == 'MMhal9000'){
  codeDir <- '~/R/R4hal/scripts/';
  outDir <- '~/R/R4hal/currentWork/socio00cnnp/' };
...
SLPsocio0 <- function( parMat, datAr, onlySLP=0, fixAcc=NA, check=1){ 
  ...
  # par. row: SEb, acc0max, acc0min, eta, gam, wexp, wrpe, Tresp, Hresp, sig e.g. parMat = c(0.73,0.81,0.47,0.04,0.66,0.05,0.04,0.12,0.42,0.08)
  ...
  for (ptN in 1:Nptot){
    ...
    We = 0.00001;          Wr = z*parMat[ptN,'wrpe']; #A: for 9p RW model just set We = 0.00001
    ...
    for (trN in 1:Ntrtot){
      ...
      # Now for the SE sum:
      SE <- as.numeric(parMat[ptN,'SEb']); #A: TO CHANGE LATER (TCL)
      j = sumN;
...
msLP0tr <- function(trParM, datAr, Pri=NA, check=1){
  ...
  if (length(Pri)>1){  # legit prior must have 12 elements or so!
    for (ptN in 1:dim(trParM)[1]) {
      ...
      mSLPrior <- mSLPrior - sum(dnorm(na.omit(parM[ptN,]),   
                                       Pri[1,],Pri[2,], log=TRUE)); 
    }
  } 
  SLP <- SLPsocio0( parM, datAr, check=1)
  predSLnP <- SLP[[1]]
  SESLnP <- SLP[[2]]
  ...
  if (mSLPrior == Inf){  # If we are in an a priori prohibited parameter region
  ...
  else if (is.na(SLP$DatBelPol[,'obsP',][2]) & 
    ...
    return ( value ); #so the lower the better
  }
...
# ################################ start of nlm fit #################################
parhd = c('SEb','acc0max','acc0min','eta', 'gam','wexp','wrpe','Tresp','Hresp','sig');
npar = length(parhd)
try({load(paste(baseDir,"loadfornlm_RW_matrix_set1.RData",sep='')); #change this depending on which set
  bestsofar <- importCSV(paste(baseDir,"ml0res1lr.csv",sep=''))})  
#this contains bestsofar9, tryPmatrix, datArW61, and priors
try({load(paste(baseDir,"/currentWork/loadfornlm_RW_matrix_set1.RData",sep='')); 
  ...
  for (ptN in pts){ #1:dim(datArW61)[3] ){
    ...
    for (set in length(attempts):1) { 
      ...
      if (vecTRUE(length(fitAttempt$estimate)>1)){
        ...
        allsets[set,1:10] <- estpOfAttempt
        allsets[set,11]   <- SLPsocio0(estpOfAttempt, D)$SESLnP
        allsets[set,12]   <- SLPsocio0(estpOfAttempt, D)$predSLnP
    ...
    ml1fit[[ptN]][[2]] <- SLPsocio0(est9p, D);
    ...
    ml1res[ptN,13]   <- round(co[1,2],digits=4);    
    #ml1res[ptN,14]   <- exp(ml1res[ptN,11]/192) 
    
    ml1res[ptN,1:10] <- tr2natLP0(ml1fit[[ptN]][[1]]$estimate);
    ml1res[ptN,11]   <- ml1fit[[ptN]][[2]][[1]];
    ml1res[ptN,12]   <- ml1fit[[ptN]][[2]][[2]];
